...
DTsim = .01; % Bin size (in fractions of a frame) for simulating model & computing likelihood
...
xlabel('time after spike (frames)');
title('basis for h');
% (Note: check out the fields of ggsim.ihbasprs to see the parameters
% controlling this basis. 
% Here, they're set to some default params)
...
if ~isempty(tsp)
    ...
    axis([0 slen, min(vmem) max(vmem*1.01)]);
end
